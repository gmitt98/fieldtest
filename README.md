...
- `judge` records the instrument that produced the scores, with `fingerprint` a short stable hash over provider, model, temperature, seed, and per-eval overrides. Two runs that both consulted a judge are not compared automatically when their fingerprints differ. A project with no `llm` evals consults none, and records `"judged": false` with no provider or model — such a run stays comparable in either direction, so adding your first `llm` eval does not discard the history of the deterministic evals beside it. Absent in runs from before v0.3. An `endpoints` key (provider name → base URL) appears inside `judge` only when the judge reached a provider configured by endpoint; with the built-in providers it is absent.
