...
def test_duplicate_eval_id_in_one_use_case_keeps_its_own_matrix_cell():
    """
    Two evals sharing an id shared a cell: `1/2` under both columns, a
    denominator larger than either eval's own n of 1, beside per-eval tables
    correctly reporting 0% of 1 and 100% of 1.
    """
    from fieldtest.results.report import format_report

    config = _duplicate_id_config()
    rows   = _duplicate_id_rows()
    report = format_report(rows, build_summary(rows, config), {}, config, "r", "full")

    block = _matrix_block(report)
    assert "1/2" not in block, f"the two evals still share a cell:\n{block}"
    assert "| f1 | 1/1 | 0/1 |" in block, block
    # And the columns say which eval each cell belongs to.
    assert "no_pii (safe)" in block and "no_pii (right)" in block, block


def test_duplicate_eval_id_is_called_out_under_the_matrix():
    """Split columns make the numbers right; the config is still ambiguous."""
    from fieldtest.results.report import format_report
    ...
    assert "declares eval id `no_pii` 2×" in report, report
...
def test_duplicate_eval_id_keeps_its_own_csv_matrix_row():
    """The CSV export accumulated on the same key and merged the same way."""
    from fieldtest.results.report import format_report_csv

    config = _duplicate_id_config()
    rows   = _duplicate_id_rows()
    csv_text = format_report_csv(rows, config)

    section = csv_text[csv_text.index("## Fixture x Eval Matrix"):]
    section = section[:section.index("## Failures")]
    assert "uc1,f1,no_pii,safe,1,1,0,1/1" in section, section
    assert "uc1,f1,no_pii,right,0,1,0,0/1" in section, section
    assert "1/2" not in section, section


def test_duplicate_eval_id_does_not_take_the_other_eval_s_type():
    """
    eval_meta was keyed by (use_case, eval_id), so a binary eval sharing an id
    with a scored one inherited is_scored from whichever was declared last: it
    reported failure_rate null *and* mean null — no number in either column,
    for an eval that failed every run.
    """
    config = _make_config([
        Eval(id="q", tag="right", type="regex", description="d",
             pattern="x", match=True),
        Eval(id="q", tag="good", type="llm", binary=False, description="d",
             scale=[1, 5], anchors={1: "bad", 5: "good"}),
    ])
    rows = [
        ResultRow(use_case="uc1", eval_id="q", tag="right", type="regex",
                  fixture_id="f1", run=1, passed=False),
        ResultRow(use_case="uc1", eval_id="q", tag="right", type="regex",
                  fixture_id="f2", run=1, passed=False),
        ResultRow(use_case="uc1", eval_id="q", tag="good", type="llm",
                  fixture_id="f1", run=1, passed=None, score=4),
    ]
    summary = build_summary(rows, config)

    assert summary["uc1"]["right"]["q"]["failure_rate"] == 1.0
    assert summary["uc1"]["good"]["q"]["mean"] == 4.0
