...
def _build_uc_section(uc, uc_rows: list[dict], uc_summary: dict) -> str:
  ...
  <div class="label-bar">
    ...
    dup_ids   = {eid for eid, _ in eval_order
                 if sum(1 for e, _ in eval_order if e == eid) > 1}
    ...
    for eid, tag in eval_order:
        ...
        shown = f"{eid} ({tag})" if eid in dup_ids else eid
        header_cells += (
            f'<th class="{tc}" data-eval-id="{_esc_py(eid)}" '
            f'data-labels="{_esc_py(labels_str)}">{_esc_py(shown)}</th>'
        )
