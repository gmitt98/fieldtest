...
def _format_fixture_matrix(
    ...
    # Eval ids are unique within a use case (enforced in config.py), so the id
    # alone identifies a column. Disambiguating by tag here would be handling a
    # state the config layer refuses to produce.
    headers = [eid for eid, _ in active]
